...
const char *RoutingTableVisualizerBase::DirectiveResolver::resolveDirective(char directive) const
{
    ...
    switch (directive) {
        ...
        case 's':
            result = route->str();
            break;
        // Custom LCS viewer 192.168.2.1 results in a 2
        case 'y':
            result = route->getDestination().isUnspecified() ? "*" : std::to_string(route->getDestination().getDByte(2));
            break;
        case 'z':
            result = route->getGateway().isUnspecified() ? "*" : std::to_string(route->getGateway().getDByte(2));
            break;
        default:
